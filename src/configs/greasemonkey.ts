...
type ModuleType = typeof import("eslint-config-greasemonkey");

/**
 * Exports the `eslint-config-greasemonkey` config.
 *
 * @returns {Promise<ImportModuleSafe<ModuleType>>}
 */
export default async function (): Promise<ImportModuleSafe<ModuleType>> {
  try {
    const module = await import("eslint-config-greasemonkey");
