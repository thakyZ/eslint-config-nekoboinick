...
const PLUGIN_KEYS = ["ts", "unused-imports"];
const CONFIG_KEYS = [
	"js_all",
	"js_recommended",
	"stylistic_all",
	"stylistic_customize",
	"stylistic_disable-legacy",
	"stylistic_recommended",
	"stylistic_all-flat",
	"stylistic_recommended-flat",
	"import-x_electron",
	"import-x_errors",
	"import-x_react",
	"import-x_recommended",
	"import-x_stage-0",
	"import-x_typescript",
	"import-x_warnings",
	"jest_all",
	"jest_recommended",
	"jest_style",
	"jsx-a11y_recommended",
	"jsx-a11y_strict",
	"markdown_recommended",
	"n_all",
	"n_mixed-esm-and-cjs",
	"n_recommended",
	"n_recommended-module",
	"n_recommended-script",
	"promise",
	"ts_all",
	"ts_base",
	"ts_disableTypeChecked",
	"ts_eslintRecommended",
	"ts_recommended",
	"ts_recommendedTypeChecked",
	"ts_recommendedTypeCheckedOnly",
	"ts_strict",
	"ts_strictTypeChecked",
	"ts_strictTypeCheckedOnly",
	"ts_stylistic",
	"ts_stylisticTypeChecked",
	"ts_stylisticTypeCheckedOnly",
	"unicorn_unopinionated",
	"unicorn_all",
	"unicorn_recommended",
	"react_all",
	"react_jsx-runtime",
	"react_recommended",
	"react-hooks_recommended",
	"react-hooks_recommended-latest",
	"react-refresh_next",
	"react-refresh_recommended",
	"react-refresh_vite",
	"react-refresh_next",
	"react-refresh_recommended",
	"react-refresh_vite",
	"next_core-web-vitals",
	"next_recommended",
	"eslint-comments_recommended"
];
const keysToRemoveIfExists = { "key-spacing": "@stylistic/key-spacing" };
export default function init(options) {
	const output = [];
	const plugins = typeof options === "function" || options?.override ? {
		jsPlugin,
		stylisticPlugin,
		importXPlugin,
		flatConfigUtils,
	...
	if (typeof options === "function") {
		...
		if (options?.plugins) {
			...
			for (const plugin of options.plugins) {
				try {
					switch (plugin) {
						case "ts":
							output.push({ plugins: { ts: tsPlugin.plugin } });
							break;
						case "unused-imports":
							output.push({ plugins: { "unused-imports": unusedImportsPlugin } });
							break;
						default:
							console.warn(`Unknown plugin option ${plugin} at index ${index}.`);
							break;
					}
				} finally {
					index++;
				...
				try {
					switch (config) {
						...
						case "import-x_electron":
							output.push(importXPlugin.configs["flat/electron"]);
							break;
						case "import-x_errors":
							output.push(importXPlugin.configs["flat/errors"]);
							break;
						case "import-x_react":
							output.push(importXPlugin.configs["flat/react"]);
							break;
						case "import-x_recommended":
							output.push(importXPlugin.configs["flat/recommended"]);
							break;
						case "import-x_stage-0":
							output.push(importXPlugin.configs["flat/stage-0"]);
							break;
						case "import-x_typescript":
							output.push(importXPlugin.configs["flat/typescript"]);
							break;
						case "import-x_warnings":
							output.push(importXPlugin.configs["flat/warnings"]);
							break;
						case "jest_all":
						...
						case "promise":
							...
							break;
						case "ts_all":
							output.push(...tsPlugin.configs.all);
							break;
						case "ts_base":
							output.push(...tsPlugin.configs.base);
							break;
						case "ts_disableTypeChecked":
							output.push(...tsPlugin.configs.disableTypeChecked);
							break;
						case "ts_eslintRecommended":
							output.push(...tsPlugin.configs.eslintRecommended);
							break;
						case "ts_recommended":
							output.push(...tsPlugin.configs.recommended);
							break;
						case "ts_recommendedTypeChecked":
							output.push(...tsPlugin.configs.recommendedTypeChecked);
							break;
						case "ts_recommendedTypeCheckedOnly":
							output.push(...tsPlugin.configs.recommendedTypeCheckedOnly);
							break;
						case "ts_strict":
							output.push(...tsPlugin.configs.strict);
							break;
						case "ts_strictTypeChecked":
							output.push(...tsPlugin.configs.strictTypeChecked);
							break;
						case "ts_strictTypeCheckedOnly":
							output.push(...tsPlugin.configs.strictTypeCheckedOnly);
							break;
						case "ts_stylistic":
							output.push(...tsPlugin.configs.stylistic);
							break;
						case "ts_stylisticTypeChecked":
							output.push(...tsPlugin.configs.stylisticTypeChecked);
							break;
						case "ts_stylisticTypeCheckedOnly":
							output.push(...tsPlugin.configs.stylisticTypeCheckedOnly);
							break;
						case "unicorn_all":
						...
						case "eslint-comments_recommended":
							...
							break;
						default:
							console.warn(`Unknown config option ${config} at index ${index}.`);
							break;
					}
				} finally {
					index++;
		...
		for (const entry of output) {
			for (const key of Object.keys(keysToRemoveIfExists)) {
				const value = keysToRemoveIfExists[key];
				if (entry.rules && value in entry.rules && key in entry.rules) {
					const value = entry.rules[key];
					switch (typeof value) {
						case "string":
							if (value !== "off") {
								entry.rules[key] = "off";
		...
		if (options?.override) {
			const override = options.override(output, plugins);
